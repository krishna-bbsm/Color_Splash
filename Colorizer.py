...
if not os.path.isfile(model):
    sg.popup_scrolled('Missing model file')
    exit()
...
def colorize_image(image_filename=None, cv2_frame=None):  # Colorizes the image provided. Can colorize either a file or a cv2 frame (read from a webcam)
...
images_col = [[sg.Image(filename='logo.png',size=(300*1,300)),sg.Text('Input file:'), sg.In(enable_events=True, key='-IN FILE-'), sg.FileBrowse(),sg.CBox('Convert to gray first',key='-MAKEGRAY-')],
              [sg.Button('Colorize', key='-PHOTO-',button_color=('gold')), sg.Button('Start Webcam', key='-WEBCAM-'), sg.Button('Save', key='-SAVE-',button_color=('green')), sg.Button('Exit',button_color=('white', 'firebrick3'),)],

              [sg.Image(filename='',key='-OUT-'),sg.Image(filename='',  key='-IN-')],]

# ----- Full layout -----
layout = [[ sg.Column(images_col),]]

# ----- Make the window -----
window = sg.Window('🅲🅾🅻🅾🆁 🆂🅿🅻🅰🆂🅷',layout, grab_anywhere=True,)
...
while True:
    ...
    if event in (None, 'Exit'):
        ...
        try:
            ...
            continue
    elif event == '-PHOTO-':        # Colorize button clicked
        try:
            if values['-IN FILE-']:
        ...
        while True:                 # Loop that reads and shows webcam until stop button is clicked
        ...
        try:
            if filename:
                ...
                prev_filename = filename
                sg.popup_quick_message('Image successfully saved', background_color='green', text_color='white', font='Any 20')
        except:
            sg.popup_quick_message('ERROR - Image NOT saved!', background_color='red', text_color='white', font='Any 20')
